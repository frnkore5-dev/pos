...
@section('title', __('sale::messages.edit_sale'))

@section('breadcrumb')
    <ol class="breadcrumb border-0 m-0">
        <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('messages.home') }}</a></li>
        <li class="breadcrumb-item"><a href="{{ route('sales.index') }}">{{ __('sale::messages.sales') }}</a></li>
        <li class="breadcrumb-item active">{{ __('sale::messages.edit') }}</li>
    </ol>
@endsection
                        ...
                        <form id="sale-form" action="{{ route('sales.update', $sale) }}" method="POST">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="status" id="status" required>
                                            <option {{ $sale->status == 'Pending' ? 'selected' : '' }} value="Pending">{{ __('sale::messages.pending') }}</option>
                                            <option {{ $sale->status == 'Shipped' ? 'selected' : '' }} value="Shipped">{{ __('sale::messages.shipped') }}</option>
                                            <option {{ $sale->status == 'Completed' ? 'selected' : '' }} value="Completed">{{ __('sale::messages.completed') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-lg-4">
                            ...
                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">
                                    {{ __('sale::messages.update_sale') }} <i class="bi bi-check"></i>
                                </button>
                            </div>
                        </form>
                    </div>
